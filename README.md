...
- 关闭标签、退出和切换工作区会询问未保存修改。**不提供崩溃草稿恢复**；正常关闭前请保存。
...
浏览器测试覆盖阅读、编辑、查找、撤销、多标签、主题和布局；桌面测试通过真实 WebView2 和 Rust 后端执行文件操作，文件选择框的返回路径由测试注入，不操作系统对话框。测试文件、截图、报告在 `artifacts/`，使用独立 WebView 配置目录，不影响正常使用设置。
...
第一版不包含云同步、插件、自动更新、导出，以及文件树中的删除/移动功能。
