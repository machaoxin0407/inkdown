...
- 新增「更多操作 → 导出为 PDF」，快捷键 Ctrl+Shift+P。直接选择保存位置，不需要安装额外浏览器。
...
下载 `Inkdown_0.1.2_x64-setup.exe` 安装，或完整解压 `Inkdown_0.1.2_x64-standalone.zip` 后运行。`Inkdown.exe` 也作为单文件程序提供；随附 LICENSE 和第三方许可声明。
